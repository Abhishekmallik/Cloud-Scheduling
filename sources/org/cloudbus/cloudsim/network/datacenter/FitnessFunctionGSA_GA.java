...
public class FitnessFunctionGSA_GA extends FitnessFunction{
	public FitnessFunctionGSA_GA(double[][] td, double[] et, double[][] vd, double[][] vt){
		super(false);
		workFlowDataTrans = td;
		workFlowTaskExcution = et;
		vmData = vd;		//0-mips, 1-exec cost
		vmTransferCost = vt;
	}
...
//			double taskCost = workFlowTaskExcution[i] / vmData[vmNum][0] * vmData[vmNum][1]; //add execution cost
			double taskCost = workFlowTaskExcution[i] / NetDatacenterBroker.mips[vmNum] * NetDatacenterBroker.executionCost[vmNum]; //add execution cost
...
//					System.out.println(vmTransferCost[vmNum][intPosition[j]]);
			...
			vmCost[vmNum] += taskDataTransferCost/10 + taskCost;
			sum += taskDataTransferCost/10 + taskCost;
...
//		return alpha*fitnessValue+(1-alpha)*(max(vmtime));
